...
Page({
  data: {
    ...
    params: {},               // 存储用户选中和输入的数据
    pickerPayMode: pickerData.PayMode,
    pickerPayModeIndex: 0,
  ...
  bindSubmit() {
    ...
    params = Object.assign({}, this.data.params, {
      PayMode: data.pickerPayMode[data.pickerPayModeIndex].value,
    });
  ...
  verifyData(data) {
    ...
    if (!_fgj.verify(data.FinalPrice, 'require')) {
      result.msg = '请填写成交价';
      return result;
    };
    if (!_fgj.verify(data.CustCommission, 'require')) {
      result.msg = '请填写客户佣金';
      return result;
    };
    if (!_fgj.verify(data.OwnerCommission, 'require')) {
      result.msg = '请填写卖方佣金';
      return result;
    };
