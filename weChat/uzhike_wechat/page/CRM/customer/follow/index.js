...
Page({
  ...
  bindSend() {
    ...
    if (data.remindWeek && !data.remindTime) {
...
/**
 * 跟进可以选择类型
 * 可以上传文字、图片、语音三种类型，三种形式必须选择一种才能发送
 * 还有一些可以选的附加功能，发送位置，提醒时间
 * 下面的功能操作栏是浮动到底部的，当输入文字，键盘弹起的时候，要浮动到键盘上面。我用的是监听输入框的事件，获取键盘的高度，修改操作栏距离底部的高度
 */